...
- app-sent Gmail SMTP reply appearing in Sent;
- Gmail threading for the app-sent reply and the following customer reply; and
...
App reply sender -> smtp.gmail.com -> customer
```
...
RT is the ticket and inbound correspondence engine. The app owns outbound
customer replies so it can preserve Gmail's exact thread headers.

- Each Gmail mailbox maps to one RT queue.
- Queue `CorrespondAddress` matches the Gmail address.
- Ticket status, inbound messages, attachments, and app-recorded reply
  correspondence live in RT.
...
- RT REST2 is used to list/search tickets, load history and attachments, update
  status, record correspondence after app-sent replies, administer queues, and
  synchronize the custom field.
...
Gmail identity. RT 6.0.3 writes a bare `ok` or `not ok` result to stderr and
does not include ticket identity in the mailgate response. The wrapper therefore
accepts success only when the combined child output contains an exact `ok` line
and no `not ok` line, then resolves the exact RFC Message-ID through REST2
`/attachments`, follows its transaction to the ticket, and verifies that the
ticket belongs to the mailbox queue. Before invoking mailgate, it performs the
same lookup so a crash after RT acceptance but before app persistence cannot
create a duplicate ticket. It exits zero only after the ticket/transaction link
is saved.
...
### App Gmail Reply Sender

App replies are sent directly by the mailbox controller through the selected
Gmail account using `smtp.gmail.com:587` with TLS. The sender loads the
mailbox's encrypted App Password server-side and never exposes it to the
client, logs, RT content, or process arguments.

Before sending, the app loads the local conversation's stored Gmail thread
metadata and selects the most recent known external Gmail Message-ID in that
thread. The outbound MIME must set:

- a new RFC Message-ID generated by the app;
- `In-Reply-To` equal to the selected prior Gmail Message-ID;
- `References` equal to the known Gmail reference chain plus the selected prior
  Gmail Message-ID; and
- a `Re:` subject preserving the original customer subject without RT's ticket
  prefix.

After Gmail accepts the SMTP message, the app performs a targeted Gmail All Mail
metadata lookup by the generated Message-ID. The reply is considered successful
only when Gmail returns a Sent message whose Gmail thread ID equals the inbound
conversation's stored Gmail thread ID. Only then does the app record the
already-sent reply in RT as an internal comment transaction tagged with the
Gmail Message-ID, persist a `GmailMessageLink` for the Sent message, and apply
the conversation label set. The app UI normalizes that linked RT comment as an
outbound app reply. RT must not send a second outbound email for app replies.
...
The local msmtp/sendmail path remains available for RT system notifications and
operational mail, but it is not the primary path for app user replies. RT
REST2 `ticket/:id/correspond` is not trusted to create Gmail-thread-correct
outbound mail because live RT 6.0.3 testing showed it rewrites the outbound
reference chain to RT pseudo-references.
...
3. Load the local conversation's Gmail thread ID, original subject, and latest
   external Gmail reference chain.
4. Send the reply through Gmail SMTP from the selected mailbox with a new
   Message-ID, `In-Reply-To`, and `References` based on that Gmail chain.
5. Look up the generated Message-ID in Gmail All Mail and require the returned
   message to be in Sent and in the same Gmail thread ID.
6. Record the accepted reply body in RT as an internal comment tagged with the
   Gmail Message-ID so RT stores the audit trail without sending a second
   outbound email.
7. Persist the outbound `GmailMessageLink`, link it to the RT transaction, and
   apply the conversation label set.

Reply failure returns a safe upstream error. It must not record a successful
audit event or UI state unless Gmail accepts the reply, Gmail confirms the Sent
thread identity, and RT accepts the app-recorded correspondence.
...
next cycle first resolves the exact RFC Message-ID through RT attachments; an
already accepted message is linked without invoking mailgate again. Unique
Gmail identity constraints prevent a retry from creating a second app link or
ticket after an ambiguous partial success.
...
- Reply from the app, verify Gmail Sent, Gmail thread identity, From/Reply-To,
  `In-Reply-To`, `References`, body, app link persistence, RT recorded
  correspondence, and attachment behavior.
...
12. App replies are sent by the selected Gmail mailbox, appear in Gmail Sent,
    remain in the correct Gmail thread, and are recorded back into RT without a
    duplicate RT outbound email.
