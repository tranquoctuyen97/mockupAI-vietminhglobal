...
import type { ControlsCopy } from "@/lib/i18n/copy";
...
export default function InternalFlagsClient({ text }: { text: ControlsCopy }) {
  ...
  const fetchFlags = useCallback(async () => {
    try {
      const res = await fetch("/api/admin/feature-flags?includeInternal=true");
      const data = await res.json();
      if (res.ok) setFlags(data.flags);
      else toast.error(data.error || "Không thể tải internal controls");
    } catch {
      toast.error("Không thể tải internal controls");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function toggleFlag(flag: FeatureFlag) {
    ...
    try {
      ...
      toast.error("Không thể cập nhật internal control");
    } finally {
      setTogglingKey(null);
    }
  }
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {flag.rolloutPercent < 100 && (
                    <span className="badge badge-warning">{flag.rolloutPercent}%</span>
                  )}
                </div>
                {flag.description && (
                  <p className="text-caption mt-1" style={{ color: "var(--text-muted)" }}>
                    <Info size={12} style={{ display: "inline", marginRight: 4, verticalAlign: "text-bottom" }} />
                    {flag.description}
                  </p>
                )}
              </div>
