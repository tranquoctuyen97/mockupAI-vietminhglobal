...
export default function Step5ContentPage() {
  ...
  async function handleSaveManual() {
    ...
    setSaveError("");
    try {
      const res = await fetch(`/api/wizard/drafts/${draftId}/save-manual`, {
        method: "POST",
      ...
      const data = await res.json();
      if (!res.ok) {
        setSaveError(data.message ?? "Không lưu được. Vui lòng thử lại.");
        return;
      }
      updateDraft({ aiContent: data.content });
      setState("ready");
    } catch {
