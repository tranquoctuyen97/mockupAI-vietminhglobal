...
async function handler(
  ...
  // Be tolerant of HTML generated by older proxy versions that included the
  // upstream `/api` segment in the rewritten path.
  const normalizedPath = path[0] === "api" ? path.slice(1) : path;
  const upstreamUrl = `${UPSTREAM}/api/${normalizedPath.join("/")}${request.nextUrl.search}`;
