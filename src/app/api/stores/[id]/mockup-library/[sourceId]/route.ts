...
export async function PATCH(
  ...
  const { id: storeId, sourceId } = await params;
  const source = await findSourceForStore(sourceId, storeId, session.tenantId);
  if (!source) return NextResponse.json({ error: "Custom mockup source not found" }, { status: 404 });
  ...
  await logAudit({
    ...
    metadata: regionChanged
      ...
      : ({ scope: "TEMPLATE", ...changedFields } as Prisma.InputJsonValue),
    ...requestInfo,
  });
...
export async function DELETE(
  ...
  await logAudit({
    ...
    resourceType: "custom_mockup_source",
    resourceId: sourceId,
    metadata: { scope: "TEMPLATE", storeId, label: source.label } as Prisma.InputJsonValue,
    ...requestInfo,
  });
...
async function findSourceForStore(sourceId: string, storeId: string, tenantId: string) {
