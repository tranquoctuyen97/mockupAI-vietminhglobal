...
import { assertColorFilterHasColors, resolveColorFilterForDraftDesign } from "./pair-color-filter";
...
test("resolveColorFilterForDraftDesign applies global color group overrides", () => {
  assert.deepEqual(
    resolveColorFilterForDraftDesign({
      draftDesignId: "draft_dark",
      selectedColorIds: ["white", "azalea"],
      storeColors: [
        { id: "white", name: "White", hex: "#FFFFFF", colorGroup: "auto" },
        { id: "azalea", name: "Azalea", hex: "#CCCCCC", colorGroup: "auto" },
      ],
      pairs: [{ lightDraftDesignId: "draft_light", darkDraftDesignId: "draft_dark" }],
      globalColorGroupOverrides: new Map([["azalea", "dark"]]),
    }),
    { colorIds: ["azalea"], colorGroup: "dark" },
  );
});
