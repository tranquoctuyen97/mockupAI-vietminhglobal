...
import { type EffectiveColorGroup, resolveColorGroups } from "@/lib/designs/color-classifier";
  ...
  const pairMockupsMatchColors = designPairs.every((pair) => {
    ...
    if (effectiveColorGroups) {
      ...
      const lightMockupColors = new Set(
        (lightJob.images ?? [])
          .filter(
            (img) => img.included && (!requireRealPrintifyMockups || isRealPrintifyMockup(img)),
          )
...
function isRealPrintifyMockup(image: {
  compositeUrl?: string | null;
  sourceUrl?: string | null;
}): boolean {
  if (
